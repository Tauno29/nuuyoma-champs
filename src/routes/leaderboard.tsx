    ...
    <div>
      <div className="mb-6 text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {judges.length} judge{judges.length === 1 ? "" : "s"} • {scores.filter((s) => s.round === displayRound).length} scores submitted
        </p>
      </div>
            ...
            <Card key={e.contestant.id} className={isTop5 ? "border-gold/60 bg-gold-soft/40" : ""}>
              <CardContent className="flex items-center gap-3 p-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">{e.judgeCount} judge{e.judgeCount === 1 ? "" : "s"} • Total {e.total}</p>
                </div>
